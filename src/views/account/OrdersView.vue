...
const orders = ref([]);
api.getOrders(token.value).then((res) => {
  orders.value = res;
  loading.value = false;
});
</script>
      ...
      <div v-if="!loading" class="row mb-3">
        <div class="container table-responsive">
          <table class="table table-hover accordion">
            ...
            <tbody id="orderTable">
              <template v-for="order in orders" :key="order.number">
                <tr
                ...
                >
                  ...
                  <td>
                    {{ DateTime.fromISO(order.date).toLocaleString(DateTime.DATETIME_SHORT) }}
                  </td>
                  <td class="text-end">{{ order.orderLines.length }}</td>
                  <td class="text-end">{{ order.total }}</td>
                </tr>
                <tr class="collapse accordion-collapse" :id="`detail-${order.number}`">
                  <td colspan="5">
